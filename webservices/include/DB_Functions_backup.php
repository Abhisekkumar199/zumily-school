...
global $conn; 
  ...
  $username = "zumily18";
  $password = "ZTIm_9ZrG&AL";
  $database = "zumily18_school";
// Create connection 
...
{
    global $conn;
    $result = mysqli_query($conn,"SELECT email_id from users WHERE email_id = '$email' and is_verified='1'");
    $no_of_rows = mysqli_num_rows($result);
    if ($no_of_rows > 0) 
    {
        // user existed 
...
{
    global $conn;
    $result = mysqli_query($conn,"SELECT phone from users WHERE phone = '$mobile' and is_phoneno_verified='1'");
    $no_of_rows = mysqli_num_rows($result);
    if ($no_of_rows > 0) 
    {
        // user existed 
 ...
 function getUserByEmailAndPassword($email, $password) 
{
    global $conn;
    $password12=md5($password);
    $result = mysqli_query($conn,"SELECT * FROM users WHERE email_id = '".$email."'  and password='".$password12."'") or die(mysql_error()); 
    // check for result  
    $no_of_rows = mysqli_num_rows($result); 
    if ($no_of_rows > 0)
    { 
        $chsql = mysqli_query($conn,"SELECT * FROM users WHERE email_id = '".$email."'"); 
        $result = mysqli_fetch_array($chsql); 
        return $result; 
    } 
    else 
 ...
 function getUserByMobileAndPassword($mobile, $password) 
{
    global $conn;
    $password12=md5($password);
    $result = mysqli_query($conn,"SELECT * FROM users WHERE phone = '".$mobile."'  and password='".$password12."'") or die(mysql_error()); 
    // check for result  
    $no_of_rows = mysqli_num_rows($result); 
    if ($no_of_rows > 0)
    { 
        $chsql = mysqli_query($conn,"SELECT * FROM users WHERE phone = '".$mobile."'"); 
        $result = mysqli_fetch_array($chsql); 
        return $result; 
    } 
    else 
...
{   
    global $conn;     
    $result = mysqli_query($conn,"SELECT * FROM users WHERE email_id = '$emailid'") or die(mysql_error()); 
    // check for result  
    $no_of_rows = mysqli_num_rows($result);
    ...
    { 
        ...
        $oror1=rand(100,999); 
        $otpcds=$oror1.$oror; 
        $resultsdf = mysqli_query($conn,"update users set otp='$otpcds' WHERE user_id = '".$result['user_id']."'") or die(mysql_error()); 
        
        
        $fromc="info@zumily.com";
        $toc=$emailid;
        $subjectc="Reset Account Password";
        $str= $result['fname']." Welcome to the zumily.com. Your OTP is ".$otpcds; 
    	send_mail($toc, $subjectc, $str, $headers1, $fromc, '');  
        return true; 
    } 
    else 
    { 
