...
$typesql = mysqli_fetch_array(mysqli_query($conn,"SELECT * FROM `users` WHERE `business_name` LIKE '%".$bussinessName."%' "));
$userId = $typesql['user_id'];
    $categorySql = mysqli_query($conn,"SELECT * FROM `flyer` WHERE `end_date`>='".$cmpdate."' and user_id = '$userId' ORDER BY `end_date` asc");
