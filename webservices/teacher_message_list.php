...
$cmpdate = strtotime(date("Y-m-d"));  
    ...
    $totalcount = mysqli_fetch_array(mysqli_query($conn,"SELECT count(m.message_id) as totalrows   FROM `messages` m inner join `message_user_delivery` ms on m.message_id = ms.message_id    WHERE ms.user_id = '".$user_id."'  {$message_type_filter}"));  
 
  
    $sql = mysqli_query($conn,"SELECT m.school_id,m.message_id,m.title,m.description,m.date_created,m.is_createdby_teacher,m.creator_id,m.total_images,m.message_images,m.total_views, m.message_type_id,m.message_type_display_name FROM `messages` m inner join `message_user_delivery` ms on m.message_id = ms.message_id  WHERE ms.user_id = '".$user_id."'  {$message_type_filter} ORDER BY m.`message_id` desc  limit $startIndex, $endIndex");   
