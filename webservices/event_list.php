...
$cmpdate = strtotime(date("Y-m-d"));  
    ...
    if(!empty($school_id))
    {
        $school_filter = "and e.school_id = '".$school_id."'";
    }
    ...
    $sql = mysqli_query($conn,"SELECT e.event_images,e.school_id,e.event_id,e.total_images,e.title,e.description,e.date_created,e.start_date,e.start_time,e.end_time,e.total_views FROM `events` e inner join `event_user_delivery` eu on e.event_id = eu.event_id   WHERE eu.user_id = '".$user_id."' {$school_filter} ORDER BY e.`event_id` desc  limit $startIndex, $endIndex");   
